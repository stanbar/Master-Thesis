...
%
\begin{abstract}
Information-centric networks introduce new vector of problems, one of them is content poisoning, which when performed successfully, can create destructive damages. Currently known protection methods like login/password, private key, biometry, SMS/email confirmation operate on the same dimension of authentication, thus can be reduced to "general private key". We propose another dimension of authentication which is time. Publisher can be authenticated only if he is able to proof access to private key, over defined period of time. In this paper we will present two implementations of such protocol, former is based on graph infection, latter is based on blockchain technology. Both implementations 
