...
That way, each node control the time of infection dissemination. If any notice that, the new certification was published earlier than previous certification + some fixed delay, it can assume that such node is cheating, and filter the certifications from such node.

Therefore we constructed new consensus algorithm where each node spread infection only iffollows those rules:
...
- if it is your home publisher, send the certification
- if the publisher contains , send the certification
