...
\section{Comparision}
Let's try to compare those two mechanisms. First of all, the GI in its raw form doesn't handle faulty nodes––both in stop and Byzantine fault form. Most of the blockchains tolerate up to $\frac{n-1}{3}$ Byzantine-faulty nodes, and Stellar FBA is not different here. So blockchain solution is better here.
...
\section{Summary}
Let's try to compare those two mechanisms. First of all, the GI in its raw form doesn't handle faulty nodes––both in stop and Byzantine fault form. Most of the blockchains tolerate up to \frac{n-1}{3} Byzantine-faulty nodes, and Stellar FBA is not different here. So blockchain solution is better here.
Another property is determinism, GA algorithm is highly random when it comes to consensus decision. Even high number of proof-of-times can sometimes result in graph extinction which we consider unwanted property. Blockchain solution on the other side is deterministic, publisher with high number of proof-of-time claims, can be completely sure sure that his content gets authenticated. Resilliency is another aspect worth comparing. GI algorithm is sensitive to both randomness, network structure, network size, $\xi$ and $Z_{IA}$ parameters. While the blockain––particulary FBA––has just one assumption about the quorums-intersection. We state that the blockchain solution is more resillient. Another a
Comparing those two mechanisms, we can notice the huge advantage 
