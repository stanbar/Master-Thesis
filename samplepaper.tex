...
\label{observations}
Running simulations give us interesting feedback.
First of all, we notice, that the parameters $Z_ia$ and $xi$ can completely change the outcome depending on the graph, if we want to make a solution that works on all types of networks, then it may be hard to find one constant value that will work for whole time. Those values should be rather dynamically changed when the size of the network changes.
For example for $\xi=0.25$, $Z_{IA}=50$, $Z_{HQ}=1$, $\tau=200$ the probability of epidemic depends mostly on the size of the graph.
