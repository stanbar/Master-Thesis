...
When we write NDO, we understand any arbitrary data that can be transferred over network: web page, music, images, video, documents and data streams. What's most important NDOs compared to traditional URLs are location independent, they don't specify the location from where the content should be served from, nor how they should be transferred to the receiver. NDO granularlity may differ from packets to data chunks to whole data objects, depending on the approach and data size.

Data naming is the most significant concept in the ICN. Data names must be unique -- similary to host names in current Internet architecture; two different DNS nodes should resolve one domain name to the same location -- two different ICN nodes must resolve one name to the same content. 
...
In Fig \ref{fig:trustgraph500} we can see the resulting trust graph for 100 nodes and $\xi = 0.5$

\begin{figure}[h!]
    \includegraphics[width=10cm]{img/webOfTrust500nodes05xi.png}
    \centering
...
\begin{figure}[h!]
    \includegraphics[width=11cm]{img/webOfTrust500nodes05xiHistogram.png}
    \centering
