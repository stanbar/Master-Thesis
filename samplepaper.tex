...
we can solve the equation by substitutingreplacing s, and solving Linear Differential Equations

\[\frac{di}{dt} = i(t) * \beta * (1 - i(t)) - \gamma * i(t)\]

\[\frac{di}{dt} = i(t) * (\beta - \beta * i(t) - \gamma)\]
\[di = i(t)\beta - i(t)^2 *\beta - i(t)*\gamma) * dt\]
\[i = \beta*1/2*i(t)^2 - \beta*1/3*i(t)^3 - \gamma1/2*i(t)^2)\]
