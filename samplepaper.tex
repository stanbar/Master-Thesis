...
Before we decide on what consensus algorithm choose, we need to outline requirements placed on such algorithm.
\begin{enumerate}
    \item \textbf{Decentralized} - Anyone can join and leave the network in anytime.
    \item \textbf{Byzantine fault tolerant} - Tolerate faulty nodes, who might stop responding or act arbitrary.
    \item \textbf{Liveness} - The system must stay available accepting new proofs-of-time.
    \item \textbf{Consistency} - The system must respond with the same state whichever node we ask.
    \item \textbf{Consistency} - The system must respond with the same state whichever node we ask.
\end{enumerate}
