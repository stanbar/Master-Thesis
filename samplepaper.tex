...
Which can't be solved analiticaly, but can be solved numerically. Here we can see the plot the function R dependent on reproduction numbercontact ratio $q = \frac{\beta}{\gamma}$:

\begin{gnuplot}[scale=0.8]
    set title '$RI_{max}(\beta)$ for $\gamma=0.2$'
	set ylabel '$R(\beta)$'
	set xlabel '$\beta$'
	set logscale x
	gamma = 0.2
...
Having the trust network model, we can try to simulate the epidemics using the graphs obtained by that model. Let's describe the algorithm used to generate 
