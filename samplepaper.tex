...
But if we increase the number of infections to one per day $\beta = 1$, then the plot look

\begin{figure}[ht]
    \includegraphics[width=9cm]{covidb1y15.png}
    \centering
    \caption{COVID-19 in SIR Model for $\beta=1$}
    \label{fig:covid2}
\end{figure} 
...
We can easly notice that, the factor determining if there will be epidemic is the rate of new infections $\frac{di}{dt}$, if it is positive, then there will be epidemic, and if it's negative, people will recover faster than spread of infection, so there will be no epidemic.
...
\[\frac{di}{dt} >= i(t) \beta s_0(t) - \gamma i(t) > 0\]


Since the number of infections $i(t)$ is never negative, we can omit it in our inequation.

\[\frac{di}{dt} >= \beta s_0(t) - \gamma > 0\]
\[ s_0 > \frac{\gamma}{\beta}\]
