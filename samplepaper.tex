...
It is hard to evaluate the protocol using common consensus requirements (safety, liveness and fault-tolerance) since they are designed for deterministic algorithms. Yet, we can be sure that in strong form of Byzantine-failure-node the protocol 
The protocol presented in current form, neither satisfy safety, liveness nor fault-tolerance properties stated for resilient consensus algorithms. Safety require that all non-faulty nodes eventually decide on the same value, we proved that if faulty node, blocks 

In this weak node-failure form, the algorithm satisfy the safety property, eventually all non-faulty nodes decide on the same value.

% Very important chapter
We don't claim that the graph infection algorithm can't guarantee those 
