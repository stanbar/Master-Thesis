...
\section{Wikipedia}
Wikipedia is great example of system that face with content posioning problem. Let's investigate how do they solve the problem. 
Everyone can become Wikipedia editor. Editor can create new article or modify existing one. But before the change is publicly available, other editors read the proposed change and vote for it's approval or propose further edit. If they can not achieve consensus, they start a discussion where they exchange their arguments, finally if they argue on one version, consensus is achieved and new version of article is published (keeping the whole history of changes).

\section{Bitcoin}
It turns out that bitcoin as a synonym of decentralization, faces simillar problem when it comes to two aspects - project development, - blockchain consensus. 
