...
So as we said previously that we will trust DavidCharlie only if majority of our friends trust him. We denote trust function as $T(n) = \%_n > \xi : (N) \rightarrow \{0,1\}$.
Let's use this formula to evaluate if $David$Charlie$ is trustworthy person. Let $\xi = 0.5$, then
\[T(DavidCharlie) = \%_{DavidCharlie} > \xi\]
We know that Alice and Bob do trust David, while Charlie don't.
\[T(David) = \%_{David} > \xi\]
\[= |F_n|/|F| > \xi\]
\[= \frac{2}{3} > \frac{1}{2}\]
\[= 1\]
