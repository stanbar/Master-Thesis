...
Fortunately something else can be done, instead of requiring the content removal. We can create the overlay network of trust, where the primitives are the trust to a content. When the trust is low, the user should be discouraged to use it (simillar how modern web browsers discourage usage of unsecure HTTP protocol to browse webpages). In such overlay trust network, each content has its Credibility Score. The Credibility Score is gained by Proving the time access to the publisher secret key proof of time.

    can assume that by default ICN are based on content caching,  thus we can not base auth proof of time, on the fact that the legit user will remove the content from the network.
Let's abstract from the idea in previous section, to the more general concept.
