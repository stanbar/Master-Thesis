...
Current internet architecture is mostly based on TCP/IP stack, which allow  establish communication channels between two IP addresses. While it worked great for past years,  it struggle to fit current demands. Today internet is dominated by transporting content such as audio, video, images and text[source] from content creators (providers) to content consumers (clients).

The Internet Protocol was designed to allow establish point-to-point communication channels. While it worked great for past years, it struggle to satisfy current demands like performance, integrity and security. Today internet is dominated by transporting content such as audio, video, images and text[source] from content creators (providers) to content consumers (clients), after (TODO some time) it turns out, that internet is used in slightly different way.  Most internet traffic is taken by content consumption. This asymetric partitioning make some people wonder if there is not better architecture to  us wonder if  
motivated creation content-based internet modele, replacing host-based m odels. some people wonder if there is not better architecture to  us wonder if  
 some people wonder if there is not bettermodtecture to  us wonder if  
