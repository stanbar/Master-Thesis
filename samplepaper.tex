...
%Second, we noticed that when the infection ends up in epidemic, it never happen after the last infectiion. In other words, we couldn't find a configuration when the epidemic happen after the last publication. Like the network would not have any momentum that would help it reach the epidemic when the publisher stop to push it. We can't proof that it's impossible, but we also can't proof that it's possible.
...
as well as in current internet scheme
\begin{figure}[h!]
