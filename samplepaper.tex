...
\[I_{max} = 7799999993.12\]

% check

We can even plot function f(x) where x is the parameter q, to see the parameter q influence the results.

\begin{gnuplot}[scale=0.8]
	set ylabel '$f(q)$'
	set xlabel '$q$'
	gamma = 0.2
	f(x) = 3 + 7.8 * 10**9 - 1/x * ( 1 + log(x * 7.8 * 10**9))
	plot [0:10] f(x) '$\frac{x0 * e^{\beta*t}}{1 - x0 + x0 * e^{\beta*t}}$'
\end{gnuplot}
...
\begin{gnuplot}[scale=0.8]
	...
	gamma = 0.2
	f(x) = 1 - 1/x * ( 1 + log(x))
	plot [1:50] f(x) '$\frac{x0 * e^{\beta*t}}{1 - x0 + x0 * e^{\beta*t}}$'
\end{gnuplot}
...
\begin{gnuplot}[scale=0.8]
	...
	gamma = 0.2
	q(x) = x/gamma
	f(x) = 1/q(x/gamma) * ( 1 + log(x/gammaq(x) * 7.8 * 10**6))
	plot [0:100.000001] f(x) '$\frac{x0 * e^{\beta*t}}{1 - x0 + x0 * e^{\beta*t}}$'
\end{gnuplot}

\begin{gnuplot}[scale=0.8]
	set ylabel '$\gamma$'
	set xlabel '$\beta$'
	set zlabel '$f(\beta,\gamma)$'
	set xrange [0:0.000005]
	set yrange [0:0.000005]
	f(x,y) = y/x * ( 1 + log(x/y * 7.8 * 10**6))
	splot f(x,y) notitle
\end{gnuplot}
