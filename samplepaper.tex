...
Damage detection is solved in cyclic polls, where each interested peer vote the hash of the storing Archive Unit (smallest smallest unit in which nodes identify each contentof storage, ) it store. Since each node consist of different set of Archive Units (AU), the protocol thread each AUrchive Unit independently. If it turns out that some node contains AU with different hash than, majority of the poll. It start sequence of repair
